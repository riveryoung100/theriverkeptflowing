...
const VERSION = "DEV-272" as const;
...
export function buildControlledExecutorInvocationAuthorizationFoundation(
  value: RiverDevControlledExecutorDispatchFoundationResult
): RiverDevControlledExecutorInvocationAuthorizationFoundationResult {
  const blockedReasons: string[] = [];

  if (value.version !== "DEV-271") {
  ...
  if (value.dispatched !== true) {
    blockedReasons.push("CONTROLLED_DISPATCH_NOT_ACCEPTED");
  }
  ...
  if (value.controlledDispatchStateOnly !== true) {
    blockedReasons.push("PREDECESSOR_NOT_CONTROLLED_DISPATCH_ONLY");
  }

  if (value.dispatchState !== "CONTROLLED_DISPATCH_ACCEPTED") {
    blockedReasons.push("INVALID_CONTROLLED_DISPATCH_STATE");
  }

  if (value.dispatchAuthorization === null) {
    blockedReasons.push("MISSING_DISPATCH_AUTHORIZATION_LINEAGE");
  }
  ...
  if (!nonEmpty(value.controlledDispatchEvidence)) {
    blockedReasons.push("MISSING_CONTROLLED_DISPATCH_EVIDENCE");
  }
  ...
  if (!predecessorAuthoritiesRemainDenied(value)) {
    blockedReasons.push("PREDECESSOR_AUTHORITY_BOUNDARY_VIOLATION");
  }

  if (value.futureExecutorInvocationBoundaryRequired !== true) {
    blockedReasons.push(
      "EXECUTOR_INVOCATION_BOUNDARY_NOT_REQUIRED"
    );
  }

  if (value.futureExecutionBoundaryRequired !== true) {
    blockedReasons.push("EXECUTION_BOUNDARY_NOT_REQUIRED");
  }
  ...
  const executorInvocationAuthorizationEvidence =
    Object.freeze([
      ...value.controlledDispatchEvidence,
      "DEV-272:CONTROLLED_DISPATCH_PREDECESSOR_VALIDATED",
      "DEV-272:EXECUTOR_INVOCATION_AUTHORIZED",
      "DEV-272:AUTHORIZATION_RESULT_INERT",
      "DEV-272:EXECUTOR_NOT_INVOKED",
      "DEV-272:OPERATION_NOT_EXECUTED",
      "DEV-272:FUTURE_CONTROLLED_EXECUTOR_INVOCATION_BOUNDARY_REQUIRED"
    ]);
  ...
  return {
    ...
    controlledDispatch: value,
    dispatchAuthorization: value.dispatchAuthorization,
