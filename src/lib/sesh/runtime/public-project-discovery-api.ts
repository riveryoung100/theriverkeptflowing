...
} from "../operations/public-project-discovery-service";
...
export interface PublicSeshProjectDiscoveryRuntimeEnvironment {
  readonly SESH_DB?:
    unknown;
}
...
export function createPublicSeshProjectDiscoveryAtRuntime(
  runtimeEnvironment:
    PublicSeshProjectDiscoveryRuntimeEnvironment,
): PublicSeshProjectDiscoveryService {
  if (
    ...
    throw new TypeError(
      "Public Sesh project discovery runtime environment is required.",
    );
  }
  ...
  return new DefaultPublicSeshProjectDiscoveryService({
    publications:
