...
string {
    ...
    return [
        ...
        "Use causes only when the source expresses a causal relationship, supports only when one candidate provides support for another, explains only when one candidate explains another, answers only when a candidate answers a question, applies-to only when the source expresses applicability, and depends-on only when the source expresses dependency.",
